...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.92\linewidth]{figures/fig_architecture}
  \caption{SiliQun system architecture. The PGIRS data plane consists of three layers: (i) the Lindblad physics engine, which models amplitude damping ($T_1$), dephasing ($T_2$), and depolarising noise calibrated to SiMOS device parameters; (ii) the tensor-network MPS/MPO simulator, which provides efficient state representation and fidelity computation; and (iii) the Gymnasium-compatible environment interface, which exposes a discrete gate-level action space to any standard DRL library. The DRL control agent (\S\ref{sec:drl_agent}) interacts with SiliQun exclusively through the Gymnasium API.}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.90\linewidth]{figures/fig_device_profiles}
  \caption{Noise parameter comparison across the four SiliQun device profiles: SiMOS (silicon metal-oxide-semiconductor quantum dot, the primary target), GAA (gate-all-around nanowire), NV (nitrogen-vacancy centre in diamond), and Ideal (noiseless reference). Each profile specifies $T_1$, $T_2$, single-qubit gate error $p_1$, and two-qubit gate error $p_2$. SiMOS parameters are calibrated to Yang~\emph{et al.}~\cite{Yang2020} and Nickl~\emph{et al.}~\cite{Nickl2025}.}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.90\linewidth]{figures/fig_env_throughput}
  \caption{Gymnasium environment throughput (steps per second) for SiliQun, Qiskit Aer, and QuTiP across qubit counts 2--8. SiliQun achieves $6\times$--$41\times$ higher throughput than Qiskit Aer and QuTiP at 4 qubits, enabling practical DRL training without GPU acceleration. All measurements on a single CPU core.}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.90\linewidth]{figures/fig_gate_accuracy}
  \caption{Analytical validation results for the SiliQun Lindblad physics engine. \textbf{(a)} Rabi oscillations: simulated $|1\rangle$ population vs.~rotation angle $\theta$ (blue) overlaid on the analytical $\sin^2(\theta/2)$ curve (red dashed). Maximum deviation: $0.00$ (machine precision). \textbf{(b)} $T_1$ amplitude decay: simulated population vs.~time (blue) vs.~$e^{-t/T_1}$ (red dashed). Maximum deviation: $2.78 \times 10^{-16}$. \textbf{(c)} Bell state fidelity vs.~coherence time under combined SiMOS noise. Maximum deviation from analytical curve: $0.0024$ over $t \in [0, 5T_2]$.}
