...
Within the Software-Defined Quantum Firmware (SDQF) architecture, SiliQun occupies the \emph{data plane}: it provides the physics simulation substrate on which DRL agents train, but does not itself implement control policy or firmware logic.
The firmware layer is ANDROMEDA~\cite{ANDROMEDA_P3}, which orchestrates multiple QUASAR~\cite{QUASAR_P2} control-plane agents into higher-level, multi-qubit behaviours; SiliQun is the environment those agents train in.
This section describes the framework in four layers: the architectural position of SiliQun within the SDQF stack (\S\ref{subsec:sdqf_arch}), the Lindblad physics engine that constitutes its computational core (\S\ref{subsec:lindblad}), the Gymnasium-compatible environment interface that exposes the physics engine to reinforcement learning agents (\S\ref{subsec:gymnasium}), and the Physics-Grounded Iterative RL Stack (PGIRS) methodology that governs how the simulator is designed and validated (\S\ref{subsec:pgirs}). Analytical validation against three closed-form benchmarks is presented in \S\ref{subsec:validation}.

%% ----------------------------------------------------------
\subsection{SDQF Architecture and the Data Plane}
\label{subsec:sdqf_arch}
%% ----------------------------------------------------------

The SDQF concept draws its organising principle from software-defined networking~\cite{McKeown2008}: just as SDN separates routing logic (control plane) from packet forwarding (data plane), SDQF separates policy synthesis from physics simulation. We formalise the SDQF system as a tuple $\mathcal{S} = (\mathcal{A}, \mathcal{E}, \mathcal{I}, \mathcal{P})$ where:
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.95\linewidth]{fig_architecture}
  \caption{\textbf{SiliQun system architecture and SDQF stack positioning.} SiliQun occupies the data plane of the Software-Defined Quantum Firmware (SDQF) architecture, providing the physics engine ($\mathcal{E}$), Gymnasium interface ($\mathcal{I}$), and device profile ($\mathcal{P}$). The control plane ($\mathcal{A}$, implemented by QUASAR~\cite{QUASAR_P2}) and firmware layer (ANDROMEDA~\cite{ANDROMEDA_P3}) interact with SiliQun exclusively through the standardised Gymnasium API. Arrows indicate data flow; dashed boundaries indicate the scope of each architectural layer.}
...
The SDQF stack as instantiated in this work has three layers above SiliQun. The QUASAR v9.51 DRL agent~\cite{QUASAR_P2} occupies the control plane, training on SiliQun to produce four-qubit gate synthesis policies at fidelity $F \geq 0.99$. The ANDROMEDA hierarchical framework~\cite{ANDROMEDA_P3} constitutes the firmware layer proper, orchestrating multiple frozen QUASAR modules through a software-defined control plane that coordinates physical quantum processing units (QPUs) without modifying the base-level policies. The logical qubit application layer sits above ANDROMEDA and is the subject of companion work~\cite{LogicalQubit_P7}. SiliQun's contribution is to make all three upper layers possible by providing a physically accurate, computationally efficient, and interface-standardised simulation substrate.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.95\linewidth]{fig_device_profiles}
  \caption{\textbf{Silicon spin qubit device profiles in SiliQun.} Comparison of $T_1$ relaxation times and $T_2^*$ dephasing times across the four device profiles supported by SiliQun: SiMOS (nominal), SiMOS-GAA (gate-all-around), Donor (phosphorus-in-silicon), and Si/SiGe (silicon-germanium heterostructure). Error bars indicate the parameter range reported in the literature for each device class~\cite{Xue2022,Noiri2022,Philips2022,Steinacker2023}. The SiMOS profile is used for all primary experiments in this paper.}
  \label{fig:device_profiles}
\end{figure}

\textbf{Simulation backend.} Gate operations are applied as exact tensor contractions on a matrix product state (MPS) representation of the $n$-qubit register, followed by singular value decomposition (SVD) truncation at a cutoff of $10^{-14}$. The maximum bond dimension is $\chi_\text{max} = 32$, which is sufficient to represent all states encountered during 4-qubit GHZ synthesis without approximation error. Circuit fidelity is computed as the squared overlap between the current state and the target state: $F = |\langle \psi_\text{target} | \psi \rangle|^2$. GPU acceleration via CuPy tensor operations achieves a $93\times$ speedup over CPU-based NumPy simulation, enabling the millions of environment steps required by ANDROMEDA's hierarchical training~\cite{ANDROMEDA_P3}.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.95\linewidth]{fig_env_throughput}
  \caption{\textbf{DRL environment throughput comparison.} Steps per second achieved by SiliQun (MPS backend), Qiskit Aer (statevector), and QuTiP (density matrix) when used as Gymnasium environments for DRL training on an Intel Xeon E5-2695 v2 CPU (single core). SiliQun maintains near-constant throughput across qubit counts owing to the MPS representation's polynomial scaling, while statevector and density matrix backends degrade exponentially. GPU-accelerated SiliQun (CuPy backend) achieves an additional $93\times$ speedup.}
  \label{fig:env_throughput}
\end{figure}
...
\textbf{Phase 4 — Hierarchical Decomposition.} For multi-qubit systems beyond the tractable horizon of flat RL agents, the synthesis problem is decomposed into a hierarchy of sub-problems. Each sub-problem is solved independently, and the solutions are composed through a principled fusion mechanism. This phase is implemented by the ANDROMEDA hierarchical framework~\cite{ANDROMEDA_P3}, which decomposes the 20-qubit surface code synthesis task into five 4-qubit sub-problems (solved by QUASAR modules) and two levels of fusion agents.
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.95\linewidth]{fig_gate_accuracy}
  \caption{\textbf{Analytical validation of the SiliQun physics engine.} Gate fidelity for all 11 gate tokens in the SiliQun action space, measured against closed-form analytical solutions. All gates achieve fidelity $F \geq 1 - 10^{-14}$, confirming machine-precision correctness of the tensor contraction implementation. The three rightmost columns show the Rabi oscillation, $T_1$ decay, and Bell state fidelity benchmarks that validate the Lindblad noise channels independently.}
  \label{fig:gate_accuracy}
\end{figure}
