...
%%   §3.2 Lindblad Engine    → P1 §3.2 (QUASAR_Paper1_Full_Manuscript.md) + P6 abstract
...
\begin{figure*}[ht]
  ...
  \caption{The Physics-Grounded Iterative Reward Stack (PGIRS) five-phase methodology. Phase~1 validates the noise model against analytical benchmarks before any agent is instantiated; Phase~2 wraps the engine in a Gymnasium interface enforcing strict data/control plane separation; Phase~3 discovers reward weights autonomously via BRFD; Phase~4 decomposes the synthesis objective into sub-goals via HMRL; Phase~5 validates the trained policy on held-out targets and across backends via QuantumExecutor. Skipping any phase risks the common failure mode of agents that achieve high simulated fidelity but fail on real hardware.}
  \label{fig:pgirs}
\end{figure*}
...
SiliQun~v3 adopts a strict three-tier package architecture that enforces
...
\begin{itemize}
  ...
  \item \texttt{siliqun.plugins.quasar} — \texttt{QuasarEnvAdapter},
...
\texttt{QuasarEnvAdapter} in a single call:

\begin{verbatim}
from siliqun.plugins import make_env
env = make_env("simos_nominal", target_state="bell")
obs, info = env.reset()
\end{verbatim}
...
\begin{figure*}[!ht]
  ...
  \caption{The SiliQun v3 three-tier package architecture.
    ...
    (\texttt{QuasarEnvAdapter}). Dependency arrows are strictly downward; Tier~1 has
...
SiliQun~v3 introduces a \texttt{TechnologyProfile} abstraction that
...
\begin{table*}[ht]
  \centering
  \caption{SiliQun v5 hardware profile parameters drawn from peer-reviewed sources.
    ...
    \textit{Note:} These values represent the target calibration parameters
    extracted from the cited experimental and simulation sources.
    The validation experiments (\S\ref{sec:experiments}) use self-contained
    inline Hamiltonians defined within each experiment script to faithfully
    replicate the exact setups of the cited papers; the framework default
    values in \texttt{siliqun.physics.noise.channels} are conservative
    approximations used for exploratory runs and may differ from the
    values shown here.}
...
\subsubsection{Worked Example: The \texttt{siliqun-gaas} Plugin}

The \texttt{siliqun-gaas} plugin, introduced alongside SiliQun~v3,
demonstrates the full plugin contract for gate-all-around (GAA) silicon
spin qubits. It is important to distinguish between two artefacts that
the plugin ships: the \emph{minimum plugin contract} and the
\emph{physics-faithful reference implementation}.

\textbf{Minimum plugin contract.}
The minimum contract required to register a new hardware platform
consists of three files totalling 47 lines of platform-specific code:
(i)~\texttt{gaa\_profile.py} (28~lines), the \texttt{GAAProfile}
subclass of \texttt{TechnologyProfile} that implements the five ABC
methods and populates the calibration record from the Tanamoto and Ono
TCAD study~\cite{Tanamoto2025};
(ii)~\texttt{gaa\_calibration.json} (12~lines), the calibration data
file with source DOIs for independent verification; and
(iii)~\texttt{gaa\_benchmarks.py} (7~lines), the three validation
benchmark calls using the shared PGIRS benchmark harness
(\S\ref{subsec:pgirs}). This 47-line contract is the \emph{only code}
a hardware integrator is required to write; the Lindblad solver,
Gymnasium interface, and RL agent are provided by the SiliQun core.

\textbf{Physics-faithful reference implementation.}
In addition to the minimum contract, \texttt{siliqun-gaas} ships a
full physics-faithful Gymnasium environment (\texttt{GaAsCNOTEnvFaithful}
and its GPU-batched variant \texttt{GaAsCNOTEnvVectorised}) that
implements the complete two-electron GaAs quantum-dot Hamiltonian,
including hyperfine fluctuations, charge noise, and FIR-filtered pulse
shaping. This reference implementation spans five files totalling
approximately 980 lines and serves as a validation benchmark for the
Lindblad solver and as a training environment for RL agents. It exceeds the minimum contract by design: it
demonstrates that the plugin interface is expressive enough to host a
research-grade quantum simulation, not merely a noise-parameter lookup.

The plugin passes all three PGIRS Phase~1 validation benchmarks with
deviations of $0.3\%$, $0.8\%$, and $1.1\%$ for the Rabi, Bell, and
$T_2$ benchmarks respectively, all within the $\pm 2\%$ tolerance.
Once registered, the environment is accessible as
\texttt{gymnasium.make("SiliQun-GAA-v1")} with no changes to the
agent, reward function, or training loop.

\subsubsection{Integration Cost and Generalisability}

The 47-line integration cost for \texttt{siliqun-gaas} establishes an
upper bound on the effort required to add a new silicon spin qubit
technology to SiliQun. More precisely, any hardware platform for which
the seven noise parameters are known from experimental characterisation
or TCAD simulation can be integrated into SiliQun in under 50 lines of
Python, without modifying the Lindblad solver, the Gymnasium interface,
or the RL agent.

This property extends, in principle, beyond silicon spin qubits.
Superconducting transmon qubits, trapped-ion systems, and neutral-atom
platforms each have well-characterised noise models expressible in the
Lindblad formalism~\cite{Krantz2019,Bruzewicz2019,Henriet2020}.
The primary adaptation required for these platforms is a
re-parameterisation of the collapse operators $\hat{L}_k$ in
Eq.~\ref{eq:lindblad}: charge-noise and exchange-noise operators are
replaced by, for example, photon-loss and dephasing operators for
superconducting qubits. The Gymnasium interface, the reward structure,
and the RL agent are platform-agnostic and require no modification.
...
The original SiliQun simulation engine represents the quantum state as a
full density matrix $\rho \in \mathbb{C}^{2^n \times 2^n}$, evolved under
the Lindblad master equation (Eq.~\ref{eq:lindblad}).
This representation scales as $O(4^n)$ in memory, imposing a hard
ceiling of $n \leq 16$ qubits on a single compute node: at $n = 16$
the density matrix occupies $\approx 68.7$\,GB, and at $n = 19$
(the ANDROMEDA v3 topology) it would require $4.4$\,TB.
This ceiling is not a fundamental physical constraint but an artefact
of the simulation representation, and it can be removed by replacing
the full density matrix with a Matrix Product Operator (MPO).
...
The bond dimension required for accurate simulation depends on the
entanglement structure of the target state.
For the GHZ state, the entanglement entropy across any bipartition is
exactly one ebit, so $\chi_{\mathrm{MPO}} = 2$ suffices for exact
representation.
For W and cluster states, the required bond dimension grows as
$O(\sqrt{n})$ and $O(n)$ respectively under short-time Lindblad
evolution with noise rate $p \sim 10^{-3}$.
In practice, $\chi_{\mathrm{MPO}} = 64$ provides fidelity $> 0.999$
relative to the exact density matrix for all target states considered
in this work at $n \leq 29$ qubits.

The backend is implemented as the \texttt{MPSLindbladEngine} class
in \texttt{siliqun\_provider/engines/mps\_engine.py}, inheriting from
the \texttt{SimulatorCore} abstract base class.
...
hardware-realistic noise validation of ANDROMEDA at 19--100 qubits
...
\begin{table}[ht]
  ...
  \caption{Memory comparison between the full density matrix (DM) and
    ...
    of the full ANDROMEDA v3 topology (19 qubits) on a single A100 GPU.}
  \label{tab:mpo_scaling}
  \begin{tabular}{lrrl}
    \hline
    $n$ & DM Memory & MPO Memory & Reduction \\ \hline
    16 (old limit) & 68.7\,GB & 4.2\,MB & $16{,}384\times$ \\
    19 (ANDROMEDA v3) & 4.4\,TB & 5.0\,MB & $883{,}000\times$ \\
    24 (ANDROMEDA v4) & 4.5\,PB & 6.3\,MB & $715\mathrm{M}\times$ \\
    29 (surface code $d{=}5$) & 4.6\,EB & 7.6\,MB & $>10^{12}\times$ \\ \hline
  \end{tabular}
\end{table}

Cross-validation experiments confirm that the \texttt{MPSLindbladEngine}
reproduces exact density matrix results for $n \leq 8$ qubits with
fidelity $> 0.999$ across all gate types (single-qubit Clifford gates,
CNOT, and all three Lindblad noise channels).
The Bell state, GHZ-3, and GHZ-5 circuits all produce probability
distributions within numerical precision of the exact density matrix,
validating the tensor contraction implementation.
This cross-validation is included in the SiliQun test suite and runs
