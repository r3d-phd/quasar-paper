...
obs, info = env.reset()
# Other supported target states: "ghz", "w", "cluster1d", "dicke_k3"
\end{verbatim}
...
simulation study~\cite{Tanamoto2026}, which, to our knowledge, offers
...
\begin{table*}[ht]
  ...
  \caption{SiliQun v5 hardware profile parameters drawn from peer-reviewed sources.
    ...
    donor~\cite{Madzik2022,Morello2020}; GAA~\cite{Tanamoto2026}.}
...
For Dicke states $|D_n^k\rangle = \binom{n}{k}^{-1/2}\sum_{|x|=k}|x\rangle$
(equal-weight superposition of all $n$-qubit basis states with exactly
$k$ excitations), the entanglement entropy across the central bipartition
scales as $O(\log n)$~\cite{Salberger2017}, so the required bond dimension
grows sub-linearly: $\chi_{\mathrm{MPO}} = O(n^{1/2})$ suffices for
fidelity $> 0.999$ at the noise rates considered here.
...
(GHZ, W, Cluster-1D, Dicke-$k$) considered in this work at $n \leq 29$ qubits.
