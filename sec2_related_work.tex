% ============================================================
% PAPER 6 — SiliQun Framework (PGIRS)
...
This section surveys each community in turn, identifies the specific gaps
that motivate SiliQun's design, and provides a detailed comparison with the
work that is most closely related to ours.
...
The application of reinforcement learning to quantum circuit synthesis has
produced a growing body of specialised training environments over the past
five years. The most mature of these is \textbf{qgym}~\cite{vanderLinde2023qgym},
...
decomposition, and circuit scheduling. qgym has attracted considerable
adoption (25 citations as of 2025) and represents the current standard for
benchmarking RL-based quantum compilers. However, qgym's noise model is
...
equation from device Hamiltonian parameters. This distinction matters for
spin-qubit research, where the relevant noise sources (charge noise,
hyperfine coupling fluctuations, spin-orbit interaction) have a
fundamentally different physical origin from the flux noise and two-level
fluctuators that dominate superconducting qubit decoherence. Qiskit Gym
also provides no SiMOS device model and is not designed for spin-qubit
control tasks.
...
\addlinespace
\textbf{PGIRS architecture} &
  Not addressed; no firmware-layer abstraction between physics and algorithm &
  Core contribution: SiliQun as the PGIRS data-plane simulator; a SAC-based DRL validation agent \\
\addlinespace
...
scientifically significant and directly relevant to the a hierarchical DRL framework
hierarchical RL pipeline described in the companion paper,
which uses SiliQun as its simulation backend. However, Zen \emph{et al.}
...
SiliQun is therefore positioned as the physics simulation layer that could
...
Table~\ref{tab:rw_comparison} summarises the position of SiliQun relative
to the full landscape of related work across five properties that we
identify as jointly necessary for a practically useful quantum DRL
simulation environment.

\begin{table}[ht]
\centering
\caption{Comparison of SiliQun with related quantum RL environments and
simulation frameworks across five jointly necessary properties. A
\checkmark\ indicates the property is fully satisfied; a $\circ$ indicates
partial satisfaction; an \texttimes\ indicates the property is absent.}
\label{tab:rw_comparison}
\small
\begin{tabular}{lccccc}
\toprule
\textbf{Framework / Work} &
  \rotatebox{75}{\textbf{Gymnasium-compatible}} &
  \rotatebox{75}{\textbf{First-principles Lindblad}} &
  \rotatebox{75}{\textbf{Hardware-specific params}} &
  \rotatebox{75}{\textbf{Gate-level synthesis}} &
  \rotatebox{75}{\textbf{Open-source / public}} \\
\midrule
...
No existing framework satisfies all five properties simultaneously.
SiliQun is the first open-source, Gymnasium-compatible simulation
environment to combine first-principles Lindblad noise, SiMOS-calibrated
hardware parameters, and gate-level circuit synthesis in a single,
reusable framework. This unique intersection defines SiliQun's scientific
