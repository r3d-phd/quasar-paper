...
\textbf{Public availability} &
  ...
  Open-source MIT licence at \url{https://github.com/r3d-phd/quasar-v6} \\
\addlinespace
...
\addlinespace
\textbf{SDQF architecture} &
  Not addressed; no firmware-layer abstraction between physics and algorithm &
  Core contribution: SiliQun as the SDQF data-plane simulator; DRL agent as control plane; ANDROMEDA as firmware layer \\
\addlinespace
...
scientifically significant and directly relevant to the ANDROMEDA
hierarchical RL pipeline described in the companion paper~\cite{ANDROMEDA_P3},
...
\textbf{Gap: Machine learning for GAA silicon spin qubits.}
To the best of our knowledge, no prior published work has applied machine learning
or deep reinforcement learning to quantum control of gate-all-around (GAA) silicon
spin qubits. Existing ML-based quantum control work targets superconducting
qubits~\cite{Niu2019,Sivak2022}, trapped-ion systems~\cite{Porotti2022}, and
SiMOS/donor silicon spin qubits~\cite{Khalid2023,Ding2023}, but not GAA platforms.
The most recent experimental characterisation of GAA silicon spin qubits is
Tanamoto~\cite{Tanamoto2026}, which reports device parameters but does not include
any ML-based control. SiliQun's Extension~3 therefore constitutes the first
published ML result on a GAA silicon spin qubit platform, enabled by the
PGIRS methodology (\S\ref{sec:pgirs}) which allows rapid extension to new
hardware platforms without algorithm redesign.
