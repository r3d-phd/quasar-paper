...
decomposition, and circuit scheduling. qgym has emerged as a widely used RL compiler benchmark in the quantum computing community~\cite{vanderLinde2023qgym}. However, qgym's noise model is
...
decoherence. Furthermore, Qiskit Gym lacks a silicon metal-oxide-semiconductor (SiMOS) device model and is not
...
on real hardware. SiliQun directly addresses this barrier in the context of silicon spin qubits, providing device-accurate Lindblad noise models for five hardware architectures: SiMOS quantum dot, GaAs singlet-triplet, phosphorus donor, gate-all-around (GAA) nanowire, and a linear-chain spin-bus topology.
...
By contrast, SiliQun operates at the gate level with discrete actions,
enabling direct integration with quantum compilers while retaining physical
noise fidelity.
...
closed-loop optimal control via GRAPE and CMA-ES on superconducting qubit
systems but does not implement standard RL algorithms and provides no
Gymnasium interface. Pulse-level
...
baselines for gate synthesis tasks and have been demonstrated on
superconducting qubit systems~\cite{Wittler2021,Sivak2022}; their application
to silicon spin qubits remains an active area of research. A direct comparison between
SiliQun-trained RL agents and GRAPE/CRAB on the SiMOS and donor profiles
constitutes important future work and is identified as the highest-priority
extension in Section~\ref{subsec:limitations}. Recent studies suggest that
RL may offer potential advantages over GRAPE not in final fidelity --- where
GRAPE is competitive --- but in robustness to time-varying noise and
adaptability to changing hardware parameters, which classical optimal control
methods cannot exploit without full re-optimisation~\cite{Sivak2022,Bolens2021}.
...
\textbf{QuTiP}~\cite{Johansson2012,Johansson2013} is the most comprehensive open-source
...
one-off QuTiP wrappers for specific tasks~\cite{Moro2021,Niu2019}, but
these target superconducting or abstract qubit models and do not incorporate
semiconductor spin-qubit Hamiltonians or noise spectra; none are publicly
released as reusable environments.
...
algorithms and quantum machine learning rather than RL gate synthesis.
...
parameters that differ substantially from the SiMOS $T_1$/$T_2^*$
...
\addlinespace
\textbf{Noise model} &
  Phenomenological: parameter-drift noise (coloured noise spectra, quasi-static Hamiltonian errors, finite bandwidth effects) appropriate for device tuning &
...
circuit execution. The two noise models are not interchangeable: the Lindblad model captures the Markovian decoherence that accumulates over a gate sequence, while the phenomenological model captures the low-frequency parameter-drift noise (quasi-static Hamiltonian errors) that dominates during slow device tuning sweeps. SiliQun's Lindblad model is therefore not a simplification of
...
\textbf{Nguyen \emph{et al.}}~\cite{Nguyen2021} (\textit{npj Quantum Information},
...
(Physical Review Research 2025~\cite{Zen2025}) demonstrated RL for the discovery of
...
QuTiP wrappers~\cite{Moro2021,Niu2019}           & \texttimes & $\circ$    & \texttimes & $\circ$    & \texttimes \\
...
Table~\ref{tab:rw_comparison} makes the gap concrete: no existing framework satisfies all five properties simultaneously. Gymnasium-compatible environments (qgym, Qiskit Gym) lack Lindblad physics; physics simulators (QuTiP wrappers) lack RL interfaces; hardware-specific tools (Orbell 2024) lack gate-synthesis support. SiliQun is the first open-source platform to close all five gaps at once, providing the physics-grounded RL interface that silicon spin qubit control research requires. We emphasise that RL is one of several viable optimisation paradigms for quantum control; its particular suitability for SiliQun's discrete gate synthesis task stems from the sequential decision-making structure and sparse reward signal, which are well-matched to policy gradient and value-based methods. Continuous parameter tuning tasks, such as those addressed by Orbell (2024), are often better served by Bayesian optimisation or gradient-based methods. This unique intersection defines SiliQun's scientific contribution and motivates the PGIRS methodology described in Section~\ref{subsec:pgirs}.
