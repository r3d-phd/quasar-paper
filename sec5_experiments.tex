...
All experiments were conducted on the King Abdulaziz University Aziz HPC cluster, using NVIDIA A100 nodes (40\,GB HBM2 VRAM) with PyTorch~2.4.0 and CUDA~12.1.
The SiliQun environment version used throughout is \texttt{siliqun-v5}
(commit \texttt{48e9949}, tag \texttt{v5}, available at
...
Table~\ref{tab:r4_results} reports the per-seed results for the three seeds
completed to date; seed 1453 is currently running on the Aziz A100 cluster
(job~179442, submitted 2~June~2026, 720~h walltime) and will be incorporated
in the final version.
Seeds 24428 and 555 achieve best-case fidelities of
$F_\text{best} = 0.8444$ and $F_\text{best} = 0.7853$ respectively.
Seed 666, completed on the Aziz A100 cluster (job~179089), achieves
$F_\text{best} = 0.8427$ over 20{,}000 episodes without converging above
$F = 0.999$.
The partial mean across the three completed seeds is
$\bar{F}_\text{partial} = 0.8269 \pm 0.0248$.

\begin{table}[ht]
\centering
\caption{Replication~4 results: TQC CNOT synthesis on SiliQun
  \texttt{gaas\_singlet\_triplet} profile.
  Seeds 24428, 555, and 666 are complete;
  seed 1453 is pending (Aziz A100, job~179442, 720~h walltime).
...
666   & $0.8427$ & $0.8427$ & No & $\approx$480 & Complete \\
1453  & \textit{pend.} & \textit{pend.} & \textit{pend.}
      & \textit{pend.} & Running (job~179442) \\
\midrule
\textbf{Mean (3 seeds)} & $\mathbf{0.8236 \pm 0.0248}$ & $0.8241$
  & 0/3 & 491 & Partial \\
\bottomrule
\end{tabular}
\end{table}
...
of this paper upon completion of Aziz job~179442 (seed~1453).
