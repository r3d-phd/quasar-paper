...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.90\linewidth]{figures/fig_speed_scaling}
  \caption{SiliQun MPS simulation throughput (environment steps per second) as a function of qubit count $n$, measured on a single CPU core. The near-linear scaling reflects the $\mathcal{O}(n\chi^3)$ complexity of MPS tensor contraction. At $n=4$ qubits (the primary experimental configuration), SiliQun achieves 2,847~steps/s, sufficient to complete 500,000 training steps in approximately 3 minutes.}
  \label{fig:speed_scaling}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=0.90\linewidth]{figures/fig_comparison}
  \caption{Speedup factor of SiliQun over Qiskit Aer and QuTiP as a function of qubit count. SiliQun achieves $6\times$--$41\times$ higher throughput at 4 qubits. The speedup increases with qubit count because QuTiP and Qiskit Aer use full density matrix representations ($4^n \times 4^n$) while SiliQun uses MPS representations with bond dimension $\chi \ll 2^n$.}
...
The primary experiment trains the DRL agent (\S\ref{sec:quasar}) on SiliQun with the 4-qubit GHZ target state $|\text{GHZ}(4)\rangle = \frac{1}{\sqrt{2}}(|0000\rangle + |1111\rangle)$. The training objective is to find a gate sequence of at most 100 steps achieving fidelity $F \geq 0.99$ under the full SiMOS Lindblad noise model.

\textbf{Convergence.} the DRL agent crosses the fault-tolerance threshold at training step 20,000, achieving $F_\text{eval} = 0.9930$ in a total wall-clock time of 194 seconds on a single A100 GPU (Table~\ref{tab:quasar_convergence}). This is the first reported result to cross $F = 0.99$ at four qubits under a first-principles SiMOS Lindblad noise model.

\begin{table}[h]
\centering
\caption{the DRL agent convergence on SiliQun v6 (4-qubit GHZ, SiMOS noise). Mean $\pm$ std over 5 seeds.}
...
\textbf{Comparison with baseline.} The Fix3 baseline (a fixed-reward-weight baseline agent) achieves a best fidelity of $F = 0.8307$ after 300,000 training steps. The DRL agent surpasses this at step 20,000 — a $15\times$ reduction in training steps and a $55\times$ reduction in wall-clock time — and continues to $F = 0.9930$, an absolute improvement of $+0.1623$ (+19.5\%). The dominant contributors are autonomous reward weight discovery, expert-guided initialisation, and hierarchical sub-goal decomposition.

\textbf{Interpretation as SiliQun validation.} The divergence between Fix3 and the DRL agent validates SiliQun on two dimensions simultaneously. First, the physics engine is accurate enough to expose the genuine difficulty of the synthesis task: if the noise model were too weak, Fix3 would have succeeded. Second, the Gymnasium interface is well-formed enough that standard RL algorithms can be applied without modification: The DRL agent uses the standard SB3 SAC implementation with no environment-specific modifications beyond the reward function.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.90\linewidth]{figures/fig_noise_overhead}
  \caption{Computational overhead of the SiliQun noise model (MPO density matrix simulation) relative to noiseless MPS statevector simulation, as a function of qubit count. The overhead ranges from 20\% at 2 qubits to 39\% at 8 qubits, confirming that the Lindblad noise model does not impose a prohibitive computational cost for DRL training at the 4-qubit scale.}
  \label{fig:noise_overhead}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=0.90\linewidth]{figures/fig_bond_dim}
  \caption{SiliQun throughput as a function of MPS bond dimension $\chi$ at $n=8$ qubits. Throughput degrades gracefully from 1,847~steps/s at $\chi=4$ to 412~steps/s at $\chi=64$. For the 4-qubit GHZ synthesis task, the maximum bond dimension encountered during training is $\chi=4$, placing SiliQun in the high-throughput regime.}
  \label{fig:bond_dim}
\end{figure}
...
T7 & SAC-Hierarchical & SAC + Reward Discovery + Hierarchy & This work (the DRL agent)                & \textbf{0.993} & \textbf{20,000} \\
\bottomrule
\end{tabular}
\end{table}
...
A key design requirement for SiliQun is that its physics engine should generalise beyond the primary SiMOS device profile to other silicon qubit architectures. We evaluate this by training the DRL agent on the GAA (Gate-All-Around) device profile, which models a nanowire-based silicon qubit with stronger confinement and correspondingly different noise parameters: $T_1 = 2$~ms, $T_2 = 1.2$~ms, $p_1 = 0.0005$, $p_2 = 0.005$. The target task is 2-qubit Bell state synthesis ($F \geq 0.99$), chosen to match the 2-qubit gate fidelity regime of current GAA experimental demonstrations.

\begin{table}[h]
\centering
\caption{the DRL agent on SiliQun GAA device profile (2-qubit Bell synthesis). Results over 5 seeds (0--4). Mean $\pm$ std computed over converged seeds; seed 4 converged during training (best $F = 1.000$) but final evaluation fidelity was $0.9705$ due to policy oscillation.}
...
To situate the DRL approach within the broader landscape of quantum optimal control, we benchmark two classical optimal control methods — GRAPE~\cite{Khaneja2005} and CRAB~\cite{Caneva2011} — on the same 4-qubit GHZ synthesis task under SiMOS Lindblad noise. Both methods are implemented via QuTiP's \texttt{pulseoptim} module~\cite{Lambert2024} with default convergence tolerances.

\begin{table}[h]
\centering
\caption{Comparison of classical optimal control (GRAPE, CRAB) and DRL (the DRL agent) on 4-qubit GHZ synthesis under SiMOS Lindblad noise. Noise-robust fidelity $F_\mathrm{nr}$ is evaluated under $1.5\times$ the training noise level. Wall-clock on A100 GPU.}
