...
across three independent seeds (42, 123, 777).
...
\textbf{Results.}
Table~\ref{tab:e4_results} and Fig.~\ref{fig:e4_bar} report the per-target results
across three independent seeds (42, 123, 456) on the \texttt{simos\_nominal} profile.
GHZ-3 achieves $\bar{F} = 0.9819 \pm 0.0255$ with 67\% of seeds reaching
$F \geq 0.99$, confirming that the biseparable structure of the GHZ state
is the most tractable target for model-free RL.
W-3 achieves $\bar{F} = 0.8490 \pm 0.0193$ (0\% above 0.99), reflecting
the genuine multipartite entanglement that requires more environment steps
to converge.
Cluster-1D achieves $\bar{F} = 0.9221 \pm 0.0426$ (0\% above 0.99),
and Dicke-$k_3$ achieves the highest fidelity of $\bar{F} = 1.0000 \pm 0.0000$
(100\% above 0.99), demonstrating that the permutation-symmetric structure
of Dicke states is particularly amenable to the BRFD reward shaping.
...
\caption{Extension~4 results: SAC multi-target state generalisation on
  ...
  $\bar{F}$ is the mean fidelity $\pm$ std across 3 seeds at end of training.}
...
\midrule
GHZ-3    & Biseparable (1 ebit)     & $0.9819 \pm 0.0255$ & $0.9916$ & 67\% \\
W-3      & Genuine multipartite     & $0.8490 \pm 0.0193$ & $0.8621$ & 0\% \\
Cluster-1D & Graph state (1D chain) & $0.9221 \pm 0.0426$ & $0.9537$ & 0\% \\
Dicke-$k_3$ & Permutation-symmetric & $\mathbf{1.0000 \pm 0.0000}$ & $1.0000$ & \textbf{100\%} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.95\columnwidth]{figures/fig_extension4_sac.pdf}
\caption{Extension~4: Mean fidelity $\pm$ std for SAC baseline across four
  target states (GHZ-3, W-3, Cluster-1D, Dicke-$k_3$) on the
  \texttt{simos\_nominal} SiMOS profile, 3 seeds each.
  Dashed line marks the $F = 0.99$ fault-tolerance threshold.
  Dicke-$k_3$ achieves perfect fidelity; GHZ-3 partially clears the
  threshold; W-3 and Cluster-1D require further training steps.}
\label{fig:e4_bar}
\end{figure}
