...
Preliminary results from ANDROMEDA~v4.6, trained on the SiliQun
\texttt{simos\_nominal} profile using a Proximal Policy Optimisation (PPO)
agent with curriculum-based progressive qubit scaling, achieve
module-level fidelity $F_\text{best} = 0.8053$ for 3-qubit GHZ state
preparation after 2\,000\,000 training steps on an NVIDIA A100 GPU,
with 4-qubit training actively in progress at the time of writing.
The hierarchical fusion protocol and full multi-qubit results will be
reported in the companion ANDROMEDA paper~\cite{ANDROMEDA_P3}.
...
SiliQun's modular design enables hierarchical composition at scale: the ANDROMEDA framework~\cite{ANDROMEDA_P3} is actively extending this by composing trained RL agent modules on SiliQun toward a multi-qubit hierarchical system, with full results to be reported separately in that work.
