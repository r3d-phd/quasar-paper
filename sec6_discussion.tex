...
from TCAD simulations~\cite{Tanamoto2026}, provides the first platform
on which such experiments can be performed.
The result ($F_\text{best} = 0.9999$, $\bar{F}_\text{cardinal} = 0.9792 \pm 0.0089$, 5 seeds)
...
as a simulation substrate by any RL-based quantum control framework. As one
example of this downstream utility, the ANDROMEDA framework~\cite{ANDROMEDA_P3}
--- a hierarchical multi-agent system for entanglement distribution in quantum
networks --- uses SiliQun as its module-level simulation environment.
ANDROMEDA's module-level agent trains directly on SiliQun's
\texttt{simos\_nominal} profile to prepare local GHZ states, and its
network-level fusion agent applies entanglement swapping to compose
module-level states into larger multi-qubit GHZ states.
This adoption validates two properties of SiliQun: first, that its
Gymnasium interface is compatible with multi-agent training frameworks
requiring parallel environment instances; and second, that the hardware
profiles calibrated for two-qubit gate synthesis generalise to the
multi-qubit state preparation tasks required by quantum network applications.
The ANDROMEDA results are reported separately in that work~\cite{ANDROMEDA_P3}.
