...
%%   §6.4 ANDROMEDA Scalability    → NEW (forward ref, placeholder for results)
...
hardware profiles and obtaining results within $0.06\%$ of the published
...
$\Delta F < 0.06\%$ relative to Daraeizadeh~et~al.~\cite{Daraeizadeh2020}.
...
\textbf{Against the Oxford DPhil spin qubit work (Orbell 2024~\cite{Orbell2024}).} Orbell's work addresses the device tuning and calibration layer of the quantum computing stack, while SiliQun addresses the gate synthesis layer. The two works are non-overlapping in problem domain, noise modelling approach, and software availability. The natural integration path is for Orbell's Bayesian optimisation pipeline to calibrate SiliQun's hardware profile from experimental data, replacing the current manually chosen conservative parameters with device-specific values. This would realise the full SDQF stack from physical hardware characterisation (Orbell's contribution) through gate synthesis (SiliQun's contribution) to hierarchical logical qubit construction (ANDROMEDA's contribution~\cite{ANDROMEDA_P3}).
...
%% ----------------------------------------------------------
\subsection{ANDROMEDA: Scaling SiliQun to Multi-Module Quantum Networks}
\label{subsec:andromeda_scalability}
%% ----------------------------------------------------------

SiliQun is designed as a modular gate-level simulation substrate, and its
most significant scalability pathway is integration with ANDROMEDA
(Autonomous Networked Distributed Reinforcement-learning Orchestration for
Multi-module Entanglement Distribution Architecture), a hierarchical
multi-agent framework for entanglement distribution in quantum networks.
ANDROMEDA decomposes the problem of preparing large-scale GHZ states across
a quantum repeater network into two levels: a module-level agent that trains
on a SiliQun instance to prepare local GHZ states of size $d$, and a
network-level fusion agent that applies entanglement swapping to merge
module-level GHZ states into a global GHZ state of size $2d$.

Preliminary results from ANDROMEDA~v4.1, trained on the SiliQun
\texttt{simos\_nominal} profile using a Soft Actor-Critic (SAC) agent,
achieve module-level fidelity $F_\text{best} = 0.8536$ for GHZ-2 state
preparation on an NVIDIA A100 GPU. The fusion protocol, which applies
Bell-basis measurements and Pauli corrections to merge two GHZ-2 modules
into a GHZ-4 state, achieves $\bar{F}_\text{fusion} = 0.311$ in the
current reactive-coordination implementation. Ongoing training with a
proactive-coordination protocol (ANDROMEDA~v4.2) is expected to improve
fusion fidelity substantially; results are pending and will be reported
in the companion ANDROMEDA paper.

The ANDROMEDA results demonstrate two important properties of SiliQun as
a simulation substrate: first, that SiliQun's Gymnasium interface is
compatible with multi-agent training frameworks that require parallel
environment instances; and second, that the hardware profiles calibrated
for single-qubit and two-qubit gate synthesis generalise to the multi-qubit
GHZ state preparation tasks required by quantum network applications.
...
SiliQun's modular design enables hierarchical composition at scale: the ANDROMEDA framework~\cite{ANDROMEDA_P3} demonstrates this by composing five trained RL agent modules trained on SiliQun into a 20-qubit hierarchical system, with full robustness results reported separately in that work.
...
can adapt online.
This comparison is the subject of ongoing work and will be reported in a
companion paper.
...
\textbf{Scaling beyond 4 qubits.} The direct training approach used in this paper (single SiliQun instance, 4-qubit GHZ target) does not scale to 8 or 16 qubits without modifications to the state representation and reward function. ANDROMEDA's hierarchical decomposition approach provides a practical path to 20 qubits, but the fusion gates between modules are themselves trained on SiliQun instances of increasing size. Extending SiliQun to support 8-qubit and 16-qubit training environments — with corresponding increases in the density matrix state space and the Lindblad solver's computational cost — is a necessary prerequisite for scaling ANDROMEDA beyond the $d = 3$ surface code.
